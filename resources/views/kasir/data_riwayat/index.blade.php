...
#tabelRiwayat tbody tr:hover { background: #e9f7ef; }
.dataTables_filter input { border: 1px solid #198754 !important; }
.page-item.active .page-link { background: #198754 !important; border-color: #198754 !important; }
.page-link { color: #198754 !important; }
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex justify-content-center gap-1">
                                <button type="button" class="btn btn-sm btn-detail btn-detail-modal" data-id="{{ $r->id }}" data-bs-toggle="modal" data-bs-target="#modalDetail" title="Detail">
                                    <i class="bi bi-eye"></i>
                                </button>
                                @if($r->status === 'paid')
                                <a href="{{ route('penjualan.cetak', $r->id) }}" class="btn btn-sm btn-outline-success" target="_blank" title="Cetak struk">
...
document.querySelectorAll('.btn-detail-modal').forEach(function (btn) {
    btn.addEventListener('click', function () {
        ...
        fetch('/kasir/riwayat/detail/' + id)
            .then(r => r.text())
            .then(html => { document.getElementById('detailPenjualan').innerHTML = html; });
    });
});
</script>
@endpush
