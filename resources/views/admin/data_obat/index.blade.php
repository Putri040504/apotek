...
<tr data-kode="{{ $o->kode_obat }}" data-barcode="{{ $o->barcode }}" data-nama="{{ $o->nama_obat }}">
...
<td>
    ...
    @if($o->stokBatches->where('jumlah', '>', 0)->count() > 1)
        <small class="text-muted d-block">{{ $o->stokBatches->where('jumlah', '>', 0)->count() }} batch</small>
    @endif
</td>
...
<button type="button"
class="btn btn-sm btn-outline-secondary btn-cetak-barcode"
data-scan-code="{{ $o->scanCodeForLabel() }}"
data-kode-internal="{{ $o->kode_obat }}"
...
$(document).on('click', '.btn-cetak-barcode', function () {
    if (!window.BarcodePrint) {
        Swal.fire({ icon: 'error', title: 'Fitur cetak tidak tersedia' });
        return;
    }
    BarcodePrint.show({
